...
public class Player {
    ...
    public Player(double posX, double posY, ArrayList<Mast> points, int player) {
        ...
        circle.setFill(AssetManagerBoids.playerSwings(playerNumber));
        playerNumber = player;
        score = 0;
    ...
    public Mast findClosestMast(ArrayList<Mast> points, double posX, double posY) {
        ...
        check.getCircle().setFill(AssetManagerBoids.mastSkin(true));
        return check;
    }
    ...
    public void update(boolean isKeyPressed) {
        if (isKeyPressed) {
            ...
            projectileMotion();
            circle.setFill(AssetManagerBoids.playerGoingUp(playerNumber, normalVelocityY < 0));
        }
        limitVelocity();
    }
