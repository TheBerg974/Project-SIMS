 ...
 * @author cstuser
 */
public class FXMLDocumentControllerSolar implements Initializable {
		...
		try {
			if (cbArrayList.size() < 2) {
				...
				if (!presetSelected) {
					switch (objectType) {
						case "planet":
							cb.setFill(AssetManagerSolar.getNeptuneImage());
							break;
						case "star":
							cb.setFill(AssetManagerSolar.getStarImage());
							break;
						case "asteroid":
							cb.setFill(AssetManagerSolar.getAsteroidImage());
							break;
					}
				} else if (presetSelected) {
					switch (presetType) {
						case "earth":
							cb.setFill(AssetManagerSolar.getEarthImage());
							break;
						case "sun":
							cb.setFill(AssetManagerSolar.getSunImage());
							break;
						case "phi":
							cb.setFill(AssetManagerSolar.getStarImage());
							break;
