...
import projectsims.ArenaSetup;
import projectsims.AssetManagerBoids;
import projectsims.Orb;
import projectsims.Predator;
import projectsims.Spike;
import projectsims.Spring;
import java.net.URL;
...
public class FXMLDocumentControllerBoids implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        panel.setBackground(AssetManagerBoids.getBackgroundImage(screenWidth + 10, screenHeight + 40));
            ...
            public void handle(long now) {
                ...
                if (counter % 6.0 == 0) {
                    ...
                    for (int x = 1; x < 5; x++) {
                        ...
                        orbBunch.get(x - 1).getCircle().setFill(AssetManagerBoids.playerOrbSprites(x, orbAnimation));
                    }
                }
                ...
                for (Spring spring : springBunch) {
                    spring.getRectangle().setFill(AssetManagerBoids.springSkin(true));
                }
                for (Mast mast : mastBunch) {
                    mast.getCircle().setFill(AssetManagerBoids.mastSkin(false));
                }
    ...
    public void animateBoidSprites(ArrayList<Boid> boid) {
        ...
        for (Boid bb : boid) {
            if (bb.gointLeft()) {
                bb.getRectangle().setFill(AssetManagerBoids.flippedBirds(animationBoid, bb.getBoidType()));
            } else {
                bb.getRectangle().setFill(AssetManagerBoids.normalBirds(animationBoid, bb.getBoidType()));
            }
        }
    }
